...
public class ChatSoundBoardListener extends ListenerAdapter {
	...
	private String botCommandCharacter = "¿";
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if ((respondToDms && event.isFromType(ChannelType.PRIVATE)) ||
            ...
            } else if(message.startsWith(botCommandCharacter)){
				playCommandInPhrase(event, requestingUser, message);
			} else {
	...
	private void playCommandInPhrase(MessageReceivedEvent event, String requestingUser, String message){
		String phraseRequested = message.substring(1);
		Optional<String> soundFileName = soundPlayer.getAvailableSoundFiles().keySet().stream().filter(phraseRequested::contains).findAny();
		if(soundFileName.isPresent()){
			LOG.info("Attempting to play file: " + soundFileName.get() + " 1 times. Requested by " + requestingUser + ".");
			soundPlayer.playFileForUser(soundFileName.get(), event.getAuthor().getName());
		}
		deleteMessage(event);
	}
